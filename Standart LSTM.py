...
model = Sequential()
# layer 1: LSTM
model.add(LSTM( input_dim=41, output_dim=100, return_sequences=True))
...
plt.show()
fig.savefig('output_load_forecasting_onehot_100.jpg', bbox_inches='tight')

# save the result into txt file
test_result = np.vstack((np.array(predicted_values).reshape(1,3415)[0], y_test)) 
np.savetxt('output_load_forecasting_result_no_features.txt', test_result)
